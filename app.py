    ...
    with col_central:
        ...
        with btn2:
            if st.button("📱 GERAR ARTES INDIVIDUAIS", type="secondary", use_container_width=True):
                ...
                base_dir = os.path.dirname(os.path.abspath(__file__))
                f_path_indiv = os.path.join(base_dir, f"Modelo_arte_individual_{opt_indiv.split()[-1]}.jpg")
        ...
        with c_prev1:
            ...
            if n_layout != 0:
                ...
                bg_p = LAYOUTS.get(n_layout, LAYOUTS[9])["bg_pattern"]
                f_path_g = os.path.join(os.path.dirname(os.path.abspath(__file__)), bg_p.format(v=opt_g.split()[-1]))
                if os.path.exists(f_path_g):
                    st.image(f_path_g, use_container_width=True)
                else:
                    st.warning(f"⚠️ Imagem não encontrada.")
            else:
        ...
        with c_prev2:
            ...
            f_path_i = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"Modelo_arte_individual_{opt_i.split()[-1]}.jpg")
            if os.path.exists(f_path_i):
                st.image(f_path_i, use_container_width=True)
            else:
                st.warning(f"⚠️ Imagem {f_path_i} faltando.")
