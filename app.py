            ...
            with c_ig:
                ...
                if caminho_img and os.path.exists(caminho_img):
                    ...
                    with st.popover("📤 Trocar", use_container_width=True):
                        st.markdown(
                            "<p style='font-size:12px; margin-bottom:5px;'>Selecione a nova foto:</p>", unsafe_allow_html=True)
                        nova_img = st.file_uploader("", type=[
                                                    'png', 'jpg', 'jpeg', 'webp'], key=f"up_trocar_{u_id}", label_visibility="collapsed")
                        if nova_img and st.button("Salvar Troca", key=f"btn_trocar_{u_id}", type="primary"):
                    ...
                    with st.popover("📤 Subir", use_container_width=True):
                        st.markdown(
                            "<p style='font-size:12px; margin-bottom:5px;'>Faça upload da imagem:</p>", unsafe_allow_html=True)
                        nova_img = st.file_uploader("", type=[
                                                    'png', 'jpg', 'jpeg', 'webp'], key=f"up_novo_{u_id}", label_visibility="collapsed")
                        if nova_img and st.button("Salvar Imagem", key=f"btn_novo_{u_id}", type="primary"):
                            salvar_imagem_upload(nova_img, cod)
                            st.rerun()
