...
def carregar_dados():
    ...
    try:
        # Força o uso do openpyxl para evitar erros silenciosos na nuvem
        df_abc = pd.read_excel(caminho_planilha, sheet_name="Curva ABC_Semanal", header=None, engine='openpyxl')
        
        # Converte a coluna B para string e procura a linha de referência
        ...
        if len(idx_tante) > 0:
            ...
            for i in range(linha_inicio, len(df_abc)):
                ...
                # Se o nome do produto estiver vazio e o código também, paramos a busca
    ...
    try:
        df_ind = pd.read_excel(
            caminho_planilha, sheet_name="Industrias", skiprows=1, header=None, engine='openpyxl')
    ...
    try:
        df_bateu = pd.read_excel(caminho_planilha, sheet_name="BateuLevou", engine='openpyxl')
    ...
    try:
        df = pd.read_excel(
            caminho_planilha, sheet_name="Banco_Dados_Semanal", skiprows=5, header=None, engine='openpyxl')
