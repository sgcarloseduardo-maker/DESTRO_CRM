...
if 'num_produtos_layout' not in st.session_state:
    st.session_state['num_produtos_layout'] = 9
...
def carregar_dados():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    arquivos_excel = glob.glob(os.path.join(
        base_dir, "*.xlsx")) + glob.glob(os.path.join(base_dir, "*.xls"))

    if not arquivos_excel:
        st.error(
            "🚨 NENHUMA PLANILHA ENCONTRADA! Coloque um arquivo Excel na pasta do sistema.")
        return pd.DataFrame()

    caminho_planilha = max(arquivos_excel, key=os.path.getmtime)
    ...
    try:
        ...
        cols_rca = [c for c in df.columns if isinstance(c, int) and c >= 13]
        freq_top = pd.Series(0, index=df.index)
        if cols_rca:
            df_rca = df[cols_rca].apply(
                pd.to_numeric, errors='coerce').fillna(0)
            if df_rca.sum().sum() > 0:
                for col in df_rca.columns:
                    top_itens = df_rca[col].nlargest(30)
                    top_itens = top_itens[top_itens > 0]
                    freq_top = freq_top.add(
                        pd.Series(1, index=top_itens.index), fill_value=0)
            else:
                freq_top = pd.Series(np.random.randint(
                    0, 100, size=len(df)), index=df.index)
        else:
            freq_top = pd.Series(np.random.randint(
                0, 100, size=len(df)), index=df.index)

        df['Freq_Top_ABC'] = freq_top
...
with st.sidebar:
    ...
    opcoes_layout = {9: "9 Espaços", 12: "12 Espaços",
    ...
    layout_selecionado = st.segmented_control(
        ...
        default=st.session_state['num_produtos_layout'],
        key=f"layout_selector_dinamico_{st.session_state['num_produtos_layout']}"
    )

    if layout_selecionado and layout_selecionado != st.session_state['num_produtos_layout']:
    ...
    with col_f1:
        filtro_abc = st.checkbox("Curva ABC (Top 30)")
    with col_f2:
...
with tab1:
    ...
    with cg1:
        if st.button("✨ TODAS CAMPANHAS (PRO)", use_container_width=True):
            ...
            add_auto_products(df_app[df_app['Curva_ABC'] == True].sort_values(
                'Freq_Top_ABC', ascending=False), 5)
    ...
    st.markdown(
        f"**Itens no painel:** {len(st.session_state['produtos_selecionados'])} / {num_produtos} espaços.")
    st.markdown("---")
    ...
    with col_central:
        ...
        n_layout = st.session_state.get('num_produtos_layout', 9)
        if n_layout not in LAYOUTS:
            n_layout = 9

        padrao_fundo = LAYOUTS[n_layout]["bg_pattern"]
        num_versao = opt_grade.split()[-1]
        fundo_grade_name = padrao_fundo.format(v=num_versao)
        ...
        f_path_indiv = os.path.join(
            base_dir, f"Modelo_arte_individual_{opt_indiv.split()[-1]}.jpg")
        ...
        if st.session_state['confirmacao_st'] is not None:
            ...
            with c_sim:
                if st.button("✅ SIM, CONTINUAR", type="primary", use_container_width=True):
                    acao, df_p, path_p = st.session_state['confirmacao_st'], st.session_state[
                        'df_pendente'], st.session_state['path_pendente']
                    st.session_state['confirmacao_st'] = None
                    if acao == 'grade':
        ...
        with c_prev1:
            st.markdown(
                f"<div style='text-align:center; font-weight:bold; color:#1E293B; margin-bottom:8px;'>Layout do Tabloide ({n_layout} Espaços)</div>", unsafe_allow_html=True)
            st.selectbox("Versão Tabloide", [
                         "Layout 1", "Layout 2", "Layout 3", "Layout 4"], key="sel_grade", label_visibility="collapsed")
            if os.path.exists(f_path_grade):
                st.image(f_path_grade, use_container_width=True)
            else:
                st.warning(f"⚠️ Imagem {fundo_grade_name} faltando.")
        ...
        with c_prev2:
            ...
            if os.path.exists(f_path_indiv):
                st.image(f_path_indiv, use_container_width=True)
            else:
                st.warning(f"⚠️ Imagem f_path_indiv faltando.")
