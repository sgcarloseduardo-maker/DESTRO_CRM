...
def gerar_pdf_planilha(df_produtos):
    ...
    rows_per_page = int((altura_util - titulo_h - header_h - 55) // row_h)
    rows_per_page = max(rows_per_page, 1)

    def quebrar_texto(texto, max_chars_l1=52, max_chars_l2=52):
    ...
    def desenhar_linha(y_top, row, idx_global):
        ...
        linhas = quebrar_texto(texto_desc)
        ...
        y_text = y_top - 18
        for linha in linhas:
            c.drawString(x_desc + 8, y_text, linha)
            y_text -= 12
    ...
    def desenhar_rodape_final(total_produtos):
        y1 = 42
        y2 = 28
        y3 = 16
        ...
        c.setFont("Helvetica", 8)
        c.drawString(
            margem_esq, y1, "*Ofertas válidas para hoje ou enquanto durarem os estoques.")
