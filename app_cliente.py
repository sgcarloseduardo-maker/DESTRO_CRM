    ...
    with hero_col2:
        ...
        if b64_logo:
            ...
            <div style='display:flex; flex-direction:column; gap:14px;'>
                <div class='destro-soft-card' style='min-height:180px; display:flex; align-items:center; justify-content:center; text-align:center;'>
                    <div>
                ...
                <div class='destro-soft-card' style='padding:18px 18px 16px 18px;'>
                    <div style='font-size:12px; font-weight:800; color:#d72638; text-transform:uppercase; letter-spacing:.08em; margin-bottom:10px;'>Como utilizar</div>
                    <div style='font-size:.97rem; line-height:1.7; color:#334155;'>Selecione os produtos pelo botão de + ou digitando o nome ou código, utilize os filtros para encontrar a marca desejada, remova itens se necessário, gere o PDF do pedido para cotação, baixe o arquivo e envie ao seu representante para receber o atendimento com mais rapidez.</div>
                </div>
            </div>
...
def carregar_dados():
    ...
    try:
        df_ind = pd.read_excel(
            caminho_planilha, sheet_name="Industrias",
    ...
    try:
        df_marcas = pd.read_excel(
            caminho_planilha, sheet_name="Marcas",
            skiprows=1, header=None, engine='openpyxl')
    ...
    try:
        df = pd.read_excel(
            caminho_planilha, sheet_name="Banco_Dados_Semanal",
...
if 'remover_cod' not in st.session_state:
    st.session_state['remover_cod'] = None


def remover_item_carrinho(codigo):
    if codigo in st.session_state['carrinho']:
        del st.session_state['carrinho'][codigo]
...
if not df_filtrado.empty:
    ...
    with col_nav2:
        st.markdown(f"<h4 style='text-align: center;'>Página {pagina_atual} de {total_paginas}</h4>",
                    unsafe_allow_html=True)
    with col_nav3:
        st.button("Próxima Página ➡️", on_click=proxima_pagina,
                  disabled=(pagina_atual == total_paginas), use_container_width=True)
    ...
    with col_nav1_bot:
        st.button("⬅️ Página Anterior ", key="ant_bot", on_click=pagina_anterior,
                  disabled=(pagina_atual == 1), use_container_width=True)
    with col_nav2_bot:
        st.markdown(f"<h4 style='text-align: center;'>Página {pagina_atual} de {total_paginas}</h4>",
                    unsafe_allow_html=True)
    with col_nav3_bot:
        st.button("Próxima Página ➡️ ", key="prox_bot", on_click=proxima_pagina,
                  disabled=(pagina_atual == total_paginas), use_container_width=True)
